...
void RegionCenter3();
void RegionCenter4();
// Is this MB per minute or Megabits per minute?
// 10 Mbit per second = 7.5 Megabytes per minute
const int MAX_WIRELESS_BANDWIDTH = 7.5;              // This is because it is known that wireless bandwidth is 10MB/sec and this is expressed in terms of MB/mins hence 600MB/min.
...
void calculate_bandwidth()
{
	...
	for(int b=1;b<=64;b++)
	{
		...
		{
		cluster_bandwidth[b] = MAX_WIRELESS_BANDWIDTH;
		}
		else
		{
			cluster_bandwidth[b] = MAX_WIRELESS_BANDWIDTH / cluster_count_migrating[b];
		}
	}
}
...
//	calculate_cong.open("test123.txt", std::fstream::app);
	...
	for (int c=1; c<=5000; c++)					//loops through all the cars 
	{
		...
		if ( cars_list[c].job_has_been_migrated==false && time+START_TIME_MIGRATION >= cars_list[c].departure_time && cars_list[c].departure_time>=time && cars_list[c].car_spot_number != -1)   // if the car is migrating				
...
int randomly_picked_car_datareplication_same_cluster (int spotnumber,int spot_number_of_this_job, int car_number_of_this_job)     //This function randomly picks a car for data replication which has a sufficient buffer time.
{
    ...
    for(int i=spotnumber;i<=spotnumber+39;i++)                         // this for loop goes through cars in the same cluster
    {
        ...
        if(cars_list[car_number_of_this_job].departure_time + BUFFER_TIME_FOR_DATA_REPLICATION <cars_list[i].departure_time )         //finds the car with a departure time of BUFFER_TIME_FOR_DATA_REPLICATION time more than the departure time of the car 
			arr[index]=i;
 ...
 */
 ...
 int max_residency_not_busy_car(int time){                       //This will find a car with the maximum dept time that doesn't have a job assigned to it.
    ...
    for ( int i=1; i<=5000; i++){
			//	outTestOutput<<"Car busy: "<<cars_list[i].busy<<" Car departure time: "<<cars_list[i].departure_time<<" Car spot number: "<<cars_list[i].car_spot_number<<" Car residency time: "<<cars_list[i].residency_time<<endl;
        if(cars_list[i].busy == false && cars_list[i].departure_time > time && cars_list[i].car_spot_number != -1 && cars_list[i].departure_time > dept_time_temp  ){
				
            dept_time_temp= cars_list[i].departure_time;
            car_num = cars_list[i].car_number;
        }
   ...
   // Time time;
    ...
    for (int i=1; i<=END_TIME; i++){  //1440
        ...
        for(int c=1; c<=5000; c++)
        {
				...
				if (  jobs_list[job_num_var].job_duration_remained >0  && cars_list[c].job_number != 0 && cars_list[c].car_flag_start_migration == false && cars_list[c].job_has_been_migrated==false && i+START_TIME_MIGRATION >= cars_list[c].departure_time && cars_list[c].departure_time >= i && cars_list[c].car_spot_number != -1)   // if the car is migrating
				{
